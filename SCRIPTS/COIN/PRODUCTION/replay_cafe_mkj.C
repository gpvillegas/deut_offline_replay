...
void replay_cafe_mkj(Int_t RunNumber = 0, Int_t MaxEvent = 0, TString ftype="") {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/%s/mkj_cafe_replay_%s_%d_%d.root";
  ...
  */
  ...
  if(gSystem->AccessPathName(ROOTFileName.Data())){
    ...
    std::cout << Form("%s already exists !  Are you sure you want to overwrite it ? [y / n)] \n (please use contrl+z instead of entering no, if you used ./run_cafe_prod.sh as gSystem->Exit(0)): ",ROOTFileName.Data());
    cin >> user_answer;
  }
  ...
  cmd = Form("mkdir -p REPORT_OUTPUT/mkj_%s", ftype.Data());  
  gSystem->Exec(cmd); // create study type dir. if it doesn't exist
  ...
  TString REPORT_FileName=Form("REPORT_OUTPUT/%s/mkj_cafe_%s_%d_%d.report", ftype.Data(), ftype.Data(), RunNumber, MaxEvent);

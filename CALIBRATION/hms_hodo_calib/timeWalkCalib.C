...
void writePlots(int runNUM)
{
  ...
  const char* dir_log = "mkdir -p output/Calibration_Plots/";
  const char* dir_log2 = "mkdir -p output/Calibration_Plots/TWpng";
  //Check if directory exists
  if (system(dir_log) != 0)
    {
      cout << "Creating Directory to store SHMS Hodo TW Calibration Plots . . ." << endl;
...
void WriteFitParam(int runNUM)
{
  ...
  TString outPar_Name = Form("output/hhodo_TWcalib_%d.param", runNUM);
...
void WriteFitParamErr(int runNUM)
{
  ...
  TString outPar_Name = Form("output/hhodo_TWcalib_Err_%d.param", runNUM); //note could put this where ever you wanted to
  outParam.open(outPar_Name);
...
using namespace std;
  ...
  TString histOutFileName = Form("output/timeWalkCalib_%d.root", run);

...
for KinFileBlock in KinFileContent.split('\n\n'):
    ...
    if int(RunNum) in range (RunStart, RunEnd+1) :  # C.Y. the +1 is to include upper bound in case RunStart=RunEnd (ie, single run)
    
        TestVar +=1
        for entry in Lines :
            if "gpbeam" in entry :
                EBeam = float((entry.split("="))[1])
            if "gtargmass_amu" in entry :
                targ_mass = float((entry.split("="))[1])
...
RunListEntry=("%.5f,%.6f,%4f,%.4f,%.7f,%.4f,%4f,%.7f" % (EBeam, targ_mass, SHMS_Angle, SHMS_P, SHMS_mass, HMS_Angle, HMS_P, HMS_mass))
print(RunListEntry)

...
# C. Yero - Feb 15, 2023
# Modified to meet the needs of deuteron 2023 Experiment.
...
# user input
REPLAY = sys.argv[1]  # "prod" or "sample
TGT    = sys.argv[2]  # "LH2" or "LD2" (targets for deuteron exp.)
ANA_CUT= sys.argv[3]  # "heep_singles" "heep_coin" or "deep"
RUNNUM = sys.argv[4]
EVTNUM = sys.argv[5]

# construct generic report output file from the user input (whihc should have been generated by baseAnalyzer.cpp)
deut_report_path = "DEUT_OUTPUT/REPORT/deut_%s_%s_%s_report_%s_%s.txt" % (REPLAY, TGT, ANA_CUT, RUNNUM, EVTNUM)

#bcm_type = sys.argv[2]         # <entry_type> = "bcm_type", passed from run_deut_prod.sh
deut_report = open(deut_report_path)
...
for line in deut_report:
    ...
    if "_Charge" in line :
        ...
        TestVar+=1
        # print(bcm_charge)
    # good elastic, or deep counts/rates
    if "real_counts" in line :
        real_counts = float((line.split(":")[1]).strip())
        TestVar+=1
        # print(heep_real)
    if "real_rate" in line :
        real_rate = float((line.split(":")[1]).strip())
        TestVar+=1
...
gen_run_info = "%i,            %s,           %s,         %s,          %d,        %s,                  %.3f,                    %.3f,                   %.3f,          %.3f          %.3f,                     %i,         %.4f,     %s,        %.6f,      %.4f,   %.3f,       %.4f,    %.3f" % \
...
# combine lists
total_list = gen_run_info_list + trig_info_list + efficiency_info_list + simc_stats_goal_info_list
...
# --- create / append data to deut runlist .csv file ----------

os.system('mkdir -p UTILS_DEUT/runlist') 
...
#fname_path_bkp='/home/cdaq/cyero/backup_runlist/deut-2022_runlist_backup.csv'
